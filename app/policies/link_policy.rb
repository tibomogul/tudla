class LinkPolicy < ApplicationPolicy
  ...
  def update?
    can_access_linkable? && link.user == user
  end
  ...
  def can_access_linkable?
    ...
    return false unless linkable_obj
    
    # For delegated types, we need to get the actual record
    linkable_record = begin
    ...
    return false unless linkable_record

    # Use class name comparison to avoid class reloading issues in development
    case linkable_record.class.name
    when "Project"
      user_has_project_access?(linkable_record)
    when "Scope"
      user_has_scope_access?(linkable_record)
    when "Task"
      user_has_task_access?(linkable_record)
    when "Team"
