class ReportsController < ApplicationController
  ...
  def ai_assist
    ...
    result = service.call(
      ...
      conversation_history: Array(params[:conversation_history]).map { |m| m.permit(:role, :content).to_h }
    )

    render json: result
  rescue ActionController::ParameterMissing
    render json: { reply: "Message is required.", updated_content: nil }, status: :bad_request
  rescue ReportAiAssistService::Error => e
    render json: { reply: e.message, updated_content: nil }, status: :internal_server_error
  end

  # POST /reports/render_markdown
  def render_markdown
    authorize :report, :render_markdown?
    content = params[:content] || ""
    html = helpers.sanitize(helpers.render_markdown_inline(content).to_s)
